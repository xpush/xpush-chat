...
ChannelServer = exports.ChannelServer = function (options, cb) {
  ...
  this.server.onConnection(function (socket) {
    ...
    console.log(' >>>>>>>>>>>>>>>> CONNECTION - ' + _query.A + " : " + _query.C + " : " + _query.U + " : " + _query.D);

    var _existedUser = self.users[_query.A + '^' + _query.U];
    if (!_existedUser) {
      mongoPersister.retrieveUser({A: _query.A, U: _query.U}, function (err, data) {

        if (err) console.error(err);

        self.users[_query.A + '^' + _query.U] = {};
        data.forEach(function (user) {

          if(_query.D == user.D){
            self.users[_query.A + '^' + _query.U][user.D] = true;
          }else{
            self.users[_query.A + '^' + _query.U][user.D] = false;
          }
    ...
    socket.on('join', function (params, callback) {
      ...
      mongoPersister.addChannelUser({
        ...
        /*
         @TODO
         채널에 Join 했는데 만약 있다면 어떻게? 모든 device 를 다 넣어야   하나? 그럼 U 로 조회하고 넣어줘 ??
         별도의 사용자 배열을 갖고 있어야 할 듯 함. 고민 필요함 !!
         이 아래는 단지 U 만 가지고 있어야 ? (어차피 core 모듈에서 저장하는데... 왜 다시 정보를 배열에 저장해야 하나? 안해도ㅗ 될 듯.
         */
        /*
         for (var x = 0; x < datas.length; x++) {
        ...
        console.log(self.server.channels);
  ...
  this.server.on('channel', function (data) {
    ...
    if (data.event == 'connect') {
      ...
      mongoPersister.retrieveUser({
        ...
        if (err) console.error(err);
        data.forEach(function (user) {
          console.log(user.A + '/' + user.U + '/' + user.D);
        });
      });
